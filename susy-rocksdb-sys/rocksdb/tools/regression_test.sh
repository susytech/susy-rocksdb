...
PSRC_PATTERN="Percentiles: P50: ([0-9\.]+) P75: ([0-9\.]+) "
PSRC_PATTERN+="P99: ([0-9\.]+) P99.9: ([0-9\.]+) P99.99: ([0-9\.]+)"
#==============================================================================
...
function update_report {
  ...
  exit_on_error $?
  psrc_statement=`cat $2 | grep Percentile`
  exit_on_error $?
  ...
  [[ $psrc_statement =~ $PSRC_PATTERN ]]
  psrc[0]=${BASH_REMATCH[1]}  # p50
  psrc[1]=${BASH_REMATCH[2]}  # p75
  psrc[2]=${BASH_REMATCH[3]}  # p99
  psrc[3]=${BASH_REMATCH[4]}  # p99.9
  psrc[4]=${BASH_REMATCH[5]}  # p99.99
  ...
  (printf "$DATA_FORMAT" \
    $COMMIT_ID $1 $REMOTE_USER_AT_HOST $NUM_MULTI_DB $NUM_KEYS $KEY_SIZE $VALUE_SIZE \
       ...
       $ops_per_s \
       $(multiply ${psrc[0]} 1000) \
       $(multiply ${psrc[1]} 1000) \
       $(multiply ${psrc[2]} 1000) \
       $(multiply ${psrc[3]} 1000) \
       $(multiply ${psrc[4]} 1000) \
       $DEBUG \
       $real_sec \
       $user_sec \
       $sys_sec \
